...
def ei_tesnor_size_update(model_parameters_dir_path, tflite_model_dir_path, mul_factor = 1.2):
    ...
    if tensor_arena_size > 0:
        ...
        print(f"Error: ARENA_SIZE not found in {model_parameters_tensor_info_file}")
        return
...
def ei_project_generate(args):
    ...
    project_example_path = prepare_ei_proj_resource(board_info, project_path, templates_path, args.ei_sdk_path, example_tmpl_dir, example_tmpl_proj, example_proj_list)
    ...
    if application_usage == 'generic':
        ...
        codegen = GenericEICodegen.from_args(project_example_path, ei_apikey, app='generic', specify_label=args.specify_label)
    else:
