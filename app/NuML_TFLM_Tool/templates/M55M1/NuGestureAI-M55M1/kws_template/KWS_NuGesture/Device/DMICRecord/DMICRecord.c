...
/*
 * Init LPPDMA for DMIC capture (no USB).
 * We use 2-descriptor scatter-gather (ping-pong) and push blocks into a ring buffer.
 */
static void LPPDMA_init(void)
{
    ...
    // Set TransMode (DMIC0 RX) with scatter-gather descriptor table
...
static volatile uint32_t s_u32PingPongIdx = 0;
...
NVT_ITCM void LPPDMA_IRQHandler(void)
{
    ...
    if (u32TDStatus & (1 << DMIC_LPPDMA_CH))
    {
        ...
        /* Ping-pong DMA: descriptor[0] -> buf0, descriptor[1] -> buf1 */
        int16_t *pi16PdmBuf = (int16_t *)s_u32PDMAPingPoneBuf_Aligned[s_u32PingPongIdx];

        /* Push captured samples into ring buffer for the application to consume */
        (void)AudioInBuf_Push(&s_sAudioBufCtrl, pi16PdmBuf, (int32_t)s_sAudioBufCtrl.u32PDMABlockSamples);
...
)
{
    ...
    /*
     * PDMA block size: use ~20ms audio blocks for smooth streaming into the ring buffer.
     * (For 16 kHz, 20ms -> 320 samples)
     */
    uint32_t u32PDMABlockSamples = u32SampleRate / 50U;
    if (u32PDMABlockSamples < 64U)
        u32PDMABlockSamples = 64U;
    if (u32PDMABlockSamples > u32BlockSamples)
        u32PDMABlockSamples = u32BlockSamples;


    //Allocate audio-in buffers
    pi16LPPDMAPingPoneBuf0 = malloc((u32PDMABlockSamples * u32Channels * sizeof(int16_t)) + (LPPDMA_BUF_ALIGH - 1));
    pi16LPPDMAPingPoneBuf1 = malloc((u32PDMABlockSamples * u32Channels * sizeof(int16_t)) + (LPPDMA_BUF_ALIGH - 1));
    ...
    s_sLPPDMA_DMIC_SCT[0].CTL = (((u32PDMABlockSamples * u32Channels) - 1) << PDMA_DSCT_CTL_TXCNT_Pos) | PDMA_WIDTH_16 | PDMA_SAR_FIX | PDMA_DAR_INC | PDMA_REQ_SINGLE | PDMA_OP_SCATTER;
    ...
    s_sLPPDMA_DMIC_SCT[1].CTL = (((u32PDMABlockSamples * u32Channels) - 1) << PDMA_DSCT_CTL_TXCNT_Pos) | PDMA_WIDTH_16 | PDMA_SAR_FIX | PDMA_DAR_INC | PDMA_REQ_SINGLE | PDMA_OP_SCATTER;
    ...
    s_sAudioBufCtrl.u32PDMABlockSamples = u32PDMABlockSamples;
...
int32_t DMICRecord_StartRec(void)
{
    if (s_sAudioBufCtrl.pi16AudioInBuf != NULL)
    {
        ...
        {
            /*
             * 64-pin package uses DMIC1_DAT/DMIC1_CLK (PB2/PB3).
             * On M55M1, this pin pair maps to DMIC channel 2.
             */
            u32ChanMask = DMIC_CTL_CHEN2_Msk;
        }
        else if (s_sAudioBufCtrl.u32Channels == 2)
        {
            /* If you ever switch to 2-ch on DMIC1 pins, it's typically CH2+CH3. */
            u32ChanMask = DMIC_CTL_CHEN2_Msk | DMIC_CTL_CHEN3_Msk;
        }
