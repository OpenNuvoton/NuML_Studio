...
static __NO_RETURN void read_sensors(void *argument)
{
    ...
    for (;;)
    {
        if (close_flag == 0U)
        {
            //printf("sensorGetStatus %u\r\n", sensorGetStatus(sensorId_accelerometer).active);
            if (sensorGetStatus(sensorId_accelerometer).active != 0U)
            {
                max_num = SENSOR_ACCELEROMETER_SIZE / SENSOR_ACCELEROMETER_NUM_PER_SAMPLE;
                num = sensorReadSamples(sensorId_accelerometer, max_num, sensorBuf, SENSOR_HARDWARE_BUF_SIZE);
...
static void recorder_start(void)
{
    ...
    recId_accelerometer = sdsRecOpen("Accelerometer",
                                     ...
                                     SDS_REC_IO_THRESHOLD_ACCELEROMETER);
...
int main(void)
{
    ...
    //      //else
    //      //{
    //          //  //printf("mpu6500_fifo_read, accel_g_sp: %f, accel_g_sp: %f\r\n", accel_g_sp[0], accel_g_sp[1]);
    //      //
    //      //}
    //    }
    //    else
