...
uint32_t sdsWrite (sdsId_t id, const void *buf, uint32_t buf_size) {
  ...
  if ((stream != NULL) && (buf != NULL) && (buf_size != 0U)) {
    ...
    //printf("stream: %u, %u cnt_free: %u\r\n", stream->idx_in, stream->cnt_in, cnt_free);
    //printf("check val: %lf, %lf, %lf\r\n", *(const float *)buf, *((const float *)buf+1), *((const float *)buf+2));
 //   printf("check stream val 1,2, 3: %lf, %lf, %lf\r\n", *((const float *)stream->buf + (stream->idx_in - num)/4) , *((const float *)stream->buf+(stream->idx_in - num)/4+1),
	//*((const float *)stream->buf+(stream->idx_in - num)/4+2));
