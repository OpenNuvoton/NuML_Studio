...
static uint32_t sdsioSend (const header_t *header, const void *data, uint32_t data_size) {
  ...
  printf("pDrvUSART->Send header: %u, data: %u\n", sizeof(header_t), data_size);
...
sdsioId_t sdsioOpen (const char *name, sdsioMode_t mode) {
  ...
  if (name != NULL) {
    ...
    if (sdsioSend(&header, name, data_size) == size) {
      ...
      uint32_t ReceiveSend = sdsioReceive(&header, NULL, 0U);
      printf("sdsioOpen ReceiveSend: %u, size: %u\n", ReceiveSend, size);
      if (ReceiveSend == size) {
...
uint32_t sdsioWrite (sdsioId_t id, const void *buf, uint32_t buf_size) {
  ...
  if ((id != NULL) && (buf != NULL) && (buf_size != 0U)) {
    ...
    uint32_t sizeSend = sdsioSend(&header, buf, buf_size);
    //printf("Write sdsioSend sizeSend: %u\n", sizeSend);
    if (sizeSend == size) {
      num = buf_size;
    }
