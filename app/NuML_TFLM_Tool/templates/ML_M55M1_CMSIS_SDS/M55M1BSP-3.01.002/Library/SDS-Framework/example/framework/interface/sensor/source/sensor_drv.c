...
sensorStatus_t sensorGetStatus(sensorId_t id)
{
    ...
    if (sensor != NULL && sensor->status.active)   // Need sensor active to read the overflow reg
    {
...
uint32_t sensorReadSamples(sensorId_t id, uint32_t num_samples, void *buf, uint32_t buf_size)
{
    ...
    if ((sensor != NULL) && (num_samples != 0U) && (buf != NULL) &&
            (buf_size >= (num_samples * sensor->config->sample_size)))   // Hardware buf_size should large than temp buf_size
    {
