...
int32_t InitGC0308_VGA_YUV422(uint32_t u32Param)
{
    ...
    uint8_t u8ID[2] = {0};
    /*
     * GC0308 control pins and I2C (SWI2C) mapping differs by board.
     *
     * - Original ObjectDetection project: PH2/PH3 + PG11/PD12
     * - 64-pin board (same as HSUSBD_Video_CAM_GC0308_CHTsai2):
     *      SCL=PC1, SDA=PC0, PWDN=PF6, nRESET=PB11
     */
#if defined(M55M1_64PIN_BOARD)
    /* SWI2C pins */
    SET_GPIO_PC1();        /* PC1 for GPIO to act as SCL */
    SET_GPIO_PC0();        /* PC0 for GPIO to act as SDA */
    ...
    SWI2C_Open(eDRVGPIO_GPIOC, eDRVGPIO_PIN1, eDRVGPIO_GPIOC, eDRVGPIO_PIN0, Delay);
#else
    ...
    /* Quick ACK check (useful for bring-up on a new board) */
    if (SWI2C_Write_8bitSlaveAddr_8bitReg_8bitData(u8DeviceID, 0xFE, 0x00) == FALSE)
    {
        printf("GC0308 I2C no ACK (addr=0x%02X). Check SCL/SDA/PWDN/nRESET pins.\n", u8DeviceID);
        return 0;
    }

    for (i = 0; i < sizeof(s_sGC0308_VGA_YUV422) / sizeof(struct NT_RegValue); i++)
    {
        SWI2C_Write_8bitSlaveAddr_8bitReg_8bitData(u8DeviceID, s_sGC0308_VGA_YUV422[i].u16RegAddr, s_sGC0308_VGA_YUV422[i].u8Value);
    }
