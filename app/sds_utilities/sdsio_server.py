...
class sdsio_manager:
    ...
    def __open(self, mode, name):
        ...
        if len(name) == 0:
            ...
            return resp_err
        invalid_chars = [chr(i) for i in range(0x00, 0x10)] + \
            [chr(0x7F), '"', '*', '/', ':', '<', '>', '?', '\\', '|']
    ...
    def execute_request(self, buf: bytes):
        ...
        if cmd == 1:
            ...
            return self.__close(sid)
        elif cmd == 3:
            return self.__write(sid, data)
        elif cmd == 4:
            return self.__read(sid, arg)
        elif cmd == 5:
            return self.__pingServer(sid)
        else:
...
class sdsio_server_serial:
    ...
    def open(self):
        ...
        try:
            ...
            #sys.exit(1)
            raise RuntimeError(f"Error initializing serial.Serial: {e}")
        ...
        while True:
            try:
                ...
                if time.time() - start_time >= self.connect_timeout:
                    ...
                    #sys.exit(1)
                    raise RuntimeError(f"Serial port open failed after {self.connect_timeout} seconds. Error: {e}")
                time.sleep(0.5)
...
def interface_validator(interface):
    try:
        ...
        pass
    raise argparse.ArgumentTypeError(
        f"Invalid network interface: {interface}!")


def parse_arguments(argv=None):
    def formatter(prog): return argparse.HelpFormatter(
        prog, max_help_position=41)
    parser = argparse.ArgumentParser(
        formatter_class=formatter, description="SDS I/O server with async I/O")
    ...
    parity_help = "Parity: " + \
        ", ".join([f"{k}={v}" for k, v in serial.PARITY_NAMES.items()])
...
async def main(argv=None):
    ...
    if args.server_type == "socket":
        ...
        try:
            ...
            print(f"Error: {e}")
            #sys.exit(1)  # No need, jump out, stop the program        
